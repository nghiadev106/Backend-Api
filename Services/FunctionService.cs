...
namespace Services
{
    ...
    public class FunctionService : IFunctionService
    {
        ...
        public void Delete(string id)
        {
            ...
            var lstChildFunction = _functionRepository.GetMulti(x => x.ParentId == id);
            if (lstChildFunction.Count()>0)
            {
                foreach(var item in lstChildFunction)
                {
                    _functionRepository.Delete(item.ID);
                }
            }
